...
class EmailService {
    constructor() {
        ...
        this.fromEmail = 'noreply@famylin.com';
    ...
    async sendSessionInviteEmail(data) {
        ...
        try {
            ...
            const formatDateForSubject = (dateString) => {
    ...
    async sendBulkSessionInvites(session, players, hostName, baseUrl) {
    ...
    async sendBulkSessionReminders(session, players, hostName, baseUrl) {
        let sent = 0;
        let failed = 0;
        for (const player of players) {
            if (!player.email) {
    ...
    generateSessionInviteHtml(data) {
    ...
    generateSessionInviteText(data) {
        const { session, player, inviteUrl, hostName } = data;
        const formatDate = (dateString) => {
            const date = new Date(dateString);
            const timezone = session.timezone || 'America/Los_Angeles';
...
If you have any questions, please contact the host directly.
    ...
    generateSessionReminderHtml(data) {
        const { session, player, inviteUrl, hostName } = data;
        const formatDate = (dateString) => {
            const date = new Date(dateString);
            const timezone = session.timezone || 'America/Los_Angeles';
    ...
    generateSessionReminderText(data) {
        const { session, player, inviteUrl, hostName } = data;
        const formatDate = (dateString) => {
            const date = new Date(dateString);
            const timezone = session.timezone || 'America/Los_Angeles';
